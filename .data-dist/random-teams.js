...
const Hazards = [
 ...
 class RandomTeams {
	...
	queryMoves(
		...
		if (!_optionalChain([moves, 'optionalAccess', _6 => _6.length])) return counter;
	...
	shouldCullAbility(
		...
		case 'Contrary': case 'Serene Grace': case 'Skill Link': case 'Strong Jaw':
			return !counter[_dex.toID.call(void 0, ability)];
		case 'Analytic':
	...
	randomSet(
		...
		do {
			...
			const runEnforcementChecker = (checkerName) => (
				_optionalChain([this, 'access', _7 => _7.moveEnforcementCheckers, 'access', _8 => _8[checkerName], 'optionalCall', _9 => _9(
					movePool, hasMove, hasAbility, hasType, counter, species , teamDetails
				)])
			);
	...
	randomTeam() {
		...
		while (pokemonPool.length && pokemon.length < 6) {
			...
			if (_optionalChain([potd, 'optionalAccess', _10 => _10.exists]) && pokemon.length === 1) species = potd;
	...
	randomBSSFactoryTeam(side, depth = 0) {
		...
		while (pokemonPool.length && pokemon.length < 6) {
			...
			for (const typeName of this.dex.types.names()) {
				...
				if (_optionalChain([resistanceAbilities, 'access', _11 => _11[abilityData.id], 'optionalAccess', _12 => _12.includes, 'call', _13 => _13(typeName)]) || !this.dex.getImmunity(typeName, types)) {
