...
var _pokemon = require('../../../sim/pokemon');
	...
	*/
	...
	satanicpanic: {
		...
		onHit(target, source) {
			...
			// @ts-ignore
			this.effect = /** @type {Effect} */ ({id: ''});
			const pokemon2 = new (0, _pokemon.Pokemon)(set2, source.side);
			this.effect = effect;
	...
	thehuntison: {
		...
		onModifyMove(move, source, target) {
			if (_optionalChain([target, 'optionalAccess', _7 => _7.beingCalledBack])) move.accuracy = true;
		},
	...
	cozycuddle: {
		...
		condition: {
			...
			onTrapPokemon(pokemon) {
				if (_optionalChain([this, 'access', _8 => _8.effectData, 'access', _9 => _9.source, 'optionalAccess', _10 => _10.isActive])) pokemon.tryTrap();
			},
		},
		secondary: null,
		target: "normal",
	...
	kipup: {
		...
		condition: {
			...
			onDamage(damage, target, source, effect) {
				...
				if (_optionalChain([effect, 'optionalAccess', _11 => _11.effectType]) === 'Move' && damage >= target.hp) {
	...
	notsoworthypirouette: {
		...
		onHit(target, source) {
			...
			const chance = _optionalChain([source, 'access', _12 => _12.volatiles, 'access', _13 => _13['notsoworthypirouette'], 'optionalAccess', _14 => _14.counter]) ? source.volatiles['notsoworthypirouette'].counter : 2;
	...
	swampyterrain: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _15 => _15.hasItem, 'call', _16 => _16('terrainextender')])) {
					return 8;
				}
				return 5;
			},
	...
	pitchblackterrain: {
		...
		condition: {
			...
			onHit(target, source, move) {
				...
				if (_optionalChain([move, 'optionalAccess', _23 => _23.effectType]) === 'Move' && move.category !== 'Status') {
			...
			onSwitchInPriority: -1,
			onSwitchIn(target) {
				if (_optionalChain([target, 'optionalAccess', _24 => _24.species, 'access', _25 => _25.name]) !== 'Mismagius') return;
			...
			onStart(battle, source, effect) {
				...
				if (_optionalChain([source, 'optionalAccess', _27 => _27.species, 'access', _28 => _28.name]) !== 'Mismagius') return;
	...
	homunculussvanity: {
		...
		self: {
			onHit(source) {
				...
				this.boost(boosts, source);
				this.add(`c|${_conditions.getName.call(void 0, 'SectoniaServant')}|Jelly baby ;w;`);
			},
		},
		secondary: null,
		target: "self",
	...
	legendaryswordsman: {
		...
		onTry(source, target) {
			...
			const move = _optionalChain([action, 'optionalAccess', _29 => _29.choice]) === 'move' ? action.move : null;
