...
// there are very few non-malicious competitive reasons to do so
 ...
 class Pokemon {
	...
	*/
	...
	getLastDamagedBy(filterOutSameSide) {
		const damagedBy = this.attackedBy.filter(
			(attacker) =>
...

 {
	...
	maxMoveDisabled(baseMove) {
		...
		if (!_optionalChain([this, 'access', _ => _.getMoveData, 'call', _2 => _2(baseMove.id), 'optionalAccess', _3 => _3.pp])) return true;
	...
	getDynamaxRequest(skipChecks) {
		...
		if (!skipChecks) {
			...
			if (this.species.cannotDynamax || _optionalChain([this, 'access', _4 => _4.illusion, 'optionalAccess', _5 => _5.species, 'access', _6 => _6.cannotDynamax])) return;
		}
 ...
 = {
	...
	transformInto(pokemon, effect) {
		...
		if (this.battle.gen === 4) {
			...
			if (this.species.num === 493) {
				...
				const targetForme = (_optionalChain([item, 'optionalAccess', _7 => _7.onPlate]) ? 'Arceus-' + item.onPlate : 'Arceus');
	...
	disableMove(moveid, isHidden, sourceEffect) {
		...
		for (const moveSlot of this.moveSlots) {
			if (moveSlot.id === moveid && moveSlot.disabled !== true) {
				...
				moveSlot.disabledSource = (_optionalChain([sourceEffect, 'optionalAccess', _8 => _8.fullname]) || moveSlot.move);
			}
		}
	}
	...
	setStatus(
		...
		if (this.status === status.id) {
			if (_optionalChain([(sourceEffect ), 'optionalAccess', _9 => _9.status]) === this.status) {
				this.battle.add('-fail', this, this.status);
			} else if (_optionalChain([(sourceEffect ), 'optionalAccess', _10 => _10.status])) {
		...
		if (!ignoreImmunities && status.id &&
				!(_optionalChain([source, 'optionalAccess', _11 => _11.hasAbility, 'call', _12 => _12('corrosion')]) && ['tox', 'psn'].includes(status.id))) {
			// the game currently never ignores immunities
			if (!this.runStatusImmunity(status.id === 'tox' ? 'psn' : status.id)) {
				this.battle.debug('immune to status');
				if (_optionalChain([(sourceEffect ), 'optionalAccess', _13 => _13.status])) {
