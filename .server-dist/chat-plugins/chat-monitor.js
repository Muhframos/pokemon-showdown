...
} exports.constructEvasionRegex = constructEvasionRegex;

 function stripWordBoundaries(regex) {
	return new RegExp(regex.toString().replace('/\\b', '').replace('\\b/iu', ''), 'iu');
} exports.stripWordBoundaries = stripWordBoundaries;
...
function saveFilters(force = false) {
 ...
 function generateRegex(word, isEvasion = false, isShortener = false, isReplacement = false) {
	try {
		if (isEvasion) {
