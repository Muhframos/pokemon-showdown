...
function runMovesearch(target, cmd, canAll, message, isTest) {
	...
	const getFullLearnsetOfPokemon = (species) => {
		...
		let usedSpeciesLearnset = _lib.Utils.deepClone(mod.getLearnsetData(usedSpecies.id));
		if (!usedSpeciesLearnset.learnset) {
			usedSpecies = _lib.Utils.deepClone(mod.getSpecies(usedSpecies.baseSpecies));
			usedSpeciesLearnset.learnset = _lib.Utils.deepClone(mod.getLearnsetData(usedSpecies.id).learnset || {});
		}
		const lsetData = new Set(Object.keys(usedSpeciesLearnset.learnset));

		while (usedSpecies.prevo) {
			usedSpecies = _lib.Utils.deepClone(mod.getSpecies(usedSpecies.prevo));
			usedSpeciesLearnset = _lib.Utils.deepClone(mod.getLearnsetData(usedSpecies.id));
...
function runLearn(target, cmd, canAll, message) {
	...
	while (targets.length) {
		...
		if (Dex.getFormat(targetid).exists) {
			...
			format = _lib.Utils.deepClone(Dex.getFormat(targetid));
			formatid = targetid;
