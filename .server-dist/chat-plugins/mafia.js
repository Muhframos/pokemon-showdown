...
class Mafia extends Rooms.RoomGame {
 ...
 const pages = {
	mafia(query, user) {
		...
		const game = _optionalChain([room, 'optionalAccess', _5 => _5.getGame, 'call', _6 => _6(Mafia)]);
 ...
 const commands = {
	mafia: {
		...
		autosub(target, room, user) {
			...
			let targetRoom = Rooms.get(args[0]);
			if (!targetRoom || targetRoom.type !== 'chat' || !targetRoom.users[user.id]) {
				if (_optionalChain([room, 'optionalAccess', _8 => _8.type]) !== 'chat') return this.errorReply(`This command is only meant to be used in chat rooms.`);
				targetRoom = room;
			} else {
				args.shift();
			}
		...
		subhost(target, room, user, connection, cmd) {
			...
			if (cmd.includes('cohost')) {
				...
				game.cohosts.push(_utils.Utils.escapeHTML(targetUser.name));
		...
		data(target, room, user, connection, cmd) {
			if (_optionalChain([room, 'optionalAccess', _9 => _9.settings, 'access', _10 => _10.mafiaDisabled])) return this.errorReply(`Mafia is disabled for this room.`);
			...
			// hack to let hosts broadcast
			const game = _optionalChain([room, 'optionalAccess', _11 => _11.getGame, 'call', _12 => _12(Mafia)]);
