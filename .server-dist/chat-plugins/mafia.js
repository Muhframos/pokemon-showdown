...
class Mafia extends Rooms.RoomGame {
	...
	sub(player, replacement) {
		...
		if (_optionalChain([newUser, 'optionalAccess', _7 => _7.connected])) {
	...
	nextSub(userid = null) {
		...
		if (!_optionalChain([sub, 'optionalAccess', _8 => _8.connected]) || !sub.named || !this.room.users[sub.id]) return; // should never happen, just to be safe
	...
	ideaDistributeRoles(user) {
		...
		for (const p in this.playerTable) {
			...
			const u = Users.get(p);
			if (_optionalChain([u, 'optionalAccess', _9 => _9.connected])) u.send(`>${this.room.roomid}\n|notify|Pick your role in the IDEA module.`);
		}
	...
	ideaPick(user, selection) {
		...
		if (!_optionalChain([this, 'access', _10 => _10.IDEA, 'optionalAccess', _11 => _11.data])) {
	...
	ideaFinalizePicks() {
		if (!_optionalChain([this, 'access', _12 => _12.IDEA, 'optionalAccess', _13 => _13.data])) {
	...
	updateHost(...hosts) {
		...
		for (const hostid of hosts) {
			const host = Users.get(hostid);
			if (!_optionalChain([host, 'optionalAccess', _14 => _14.connected])) return;
	...
	canJoin(user, self = false, force = false) {
		if (!_optionalChain([user, 'optionalAccess', _15 => _15.connected])) return `User not found.`;
	...
	sendUser(user, message) {
		...
		if (!_optionalChain([userObject, 'optionalAccess', _16 => _16.connected])) return;
 ...
 const pages = {
	mafia(query, user) {
		...
		const game = _optionalChain([room, 'optionalAccess', _17 => _17.getGame, 'call', _18 => _18(Mafia)]);
		if (!_optionalChain([room, 'optionalAccess', _19 => _19.users, 'access', _20 => _20[user.id]]) || !game || game.ended) {
			return this.close();
		}
	...
	mafia: {
		...
		host(target, room, user, connection, cmd) {
			...
			if (nextHost) {
				...
				while ((hostid = hostQueue.shift())) {
					...
					if (!_optionalChain([this, 'access', _21 => _21.targetUser, 'optionalAccess', _22 => _22.connected]) ||
			...
			if (!_optionalChain([this, 'access', _23 => _23.targetUser, 'optionalAccess', _24 => _24.connected])) {
		...
		queue(target, room, user) {
			...
			case 'add':
				...
				if ((!_optionalChain([targetUser, 'optionalAccess', _25 => _25.connected])) && !command.includes('force')) {
		...
		subhost(target, room, user, connection, cmd) {
			...
			const targetUser = this.targetUser;
			if (!_optionalChain([targetUser, 'optionalAccess', _26 => _26.connected])) {
				const targetUsername = this.targetUsername;
				return this.errorReply(`The user "${targetUsername}" was not found.`);
			}
		...
		data(target, room, user, connection, cmd) {
			if (_optionalChain([room, 'optionalAccess', _27 => _27.settings, 'access', _28 => _28.mafiaDisabled])) return this.errorReply(`Mafia is disabled for this room.`);
			...
			// hack to let hosts broadcast
			const game = _optionalChain([room, 'optionalAccess', _29 => _29.getGame, 'call', _30 => _30(Mafia)]);
