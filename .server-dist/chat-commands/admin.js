...
async function rebuild(context) {
 ...
 const commands = {
	...
	sendhtmlpage(target, room, user) {
		room = this.requireRoom();
		this.checkCan('addhtml', null, room);
		let [targetID, pageid, content] = _lib.Utils.splitFirst(target, ',', 2);
		if (!target || !pageid || !content) return this.parse(`/help sendhtmlpage`);

		pageid = `${user.id}-${toID(pageid)}`;

		const targetUser = Users.get(targetID);
		if (!_optionalChain([targetUser, 'optionalAccess', _2 => _2.connected])) {
	...
	highlighthtmlpage(target, room, user) {
		target = target.trim();
		let [userid, pageid, title, highlight] = _lib.Utils.splitFirst(target, ',', 3);
		pageid = `${user.id}-${toID(pageid)}`;
		if (!userid || !pageid || !target) return this.parse(`/help highlighthtmlpage`);
		const targetUser = Users.get(userid);
		if (!_optionalChain([targetUser, 'optionalAccess', _3 => _3.connected])) {
	...
	async savelearnsets(target, room, user, connection) {
		...
		await _lib.FS.call(void 0, 'data/learnsets.js').write(`'use strict';\n\nexports.Learnsets = {\n` +
			Object.entries(Dex.data.Learnsets).map(([id, entry]) => (
				`\t${id}: {learnset: {\n` +
				Object.entries(Dex.species.getLearnsetData(id )).sort(
	...
	kill(target, room, user) {
		...
		if (!_optionalChain([logRoom, 'optionalAccess', _4 => _4.log, 'access', _5 => _5.roomlogStream])) return process.exit();
	...
	async eval(target, room, user, connection) {
		...
		_optionalChain([logRoom, 'optionalAccess', _6 => _6.roomlog, 'call', _7 => _7(`>> ${target}`)]);
		let uhtmlId = null;
		try {
			...
			if (_optionalChain([result, 'optionalAccess', _8 => _8.then])) {
			...
			_optionalChain([logRoom, 'optionalAccess', _9 => _9.roomlog, 'call', _10 => _10(`<< ${result}`)]);
		} catch (e) {
			...
			_optionalChain([logRoom, 'optionalAccess', _11 => _11.roomlog, 'call', _12 => _12(`<< ${message}`)]);
		}
	},
